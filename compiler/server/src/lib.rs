...
use lushui_diagnostics::{reporter::Buffer, Diagnostic, ErrorCode, Reporter, UntaggedDiagnostic};
use lushui_error::Result;
use lushui_hir as hir;
use lushui_package::resolve_file;
use lushui_resolver::ProgramEntryExt;
use lushui_session::{Component, ComponentType, Components, BuildSession};
use lushui_span::{ByteIndex, SourceMap, Spanning};
use lushui_utilities::{HashMap, IOError};
use span::{ToLocationExt, FromPositionExt};
use std::{
...
struct Server {
    ...
    // @Beacon @Beacon @Beacon @Task remove this, we don't need this!! we have a frckin SourceMap here!!!
...
// @Beacon @Beacon @Beacon @Task instead of a path and the content, take a SourceFileIndex!!!
fn check_file(
    path: &Path,
    content: Arc<String>,
    map: &Arc<RwLock<SourceMap>>,
...
fn build_component(
    ...
    let component_root = lushui_lowerer::lower_file(
        component_root,
        lushui_lowerer::Options {
    ...
    let component_root = lushui_resolver::resolve_declarations(component_root, component, session)?;
