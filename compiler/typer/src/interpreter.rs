...
use lushui_ast::Explicit;
use lushui_diagnostics::Diagnostic;
use lushui_entity::Entity;
use lushui_error::{PossiblyErroneous, Result};
use lushui_hir::{
    self as hir, known, DeBruijnIndex, DeclarationIndex, Display,
    Identifier, Substitution::Shift, ValueView,
};
use lushui_lowered_ast::Attributes;
use lushui_resolver::ProgramEntryExt;
use lushui_session::{Component, DeclarationIndexExt, BuildSession};
use lushui_utilities::{formatted, AsDebug};
...
impl<'a> Interpreter<'a> {
    ...
    pub(crate) fn apply_intrinsic_function(
        ...
        match self.look_up(binder.declaration_index().unwrap()).kind {
            lushui_entity::EntityKind::IntrinsicFunction {
                arity, function, ..
...
impl BindingRegistration {
    fn format<C: hir::DisplayContext>(
        &self,
        context: C,
        ...
        match &self.bare {
            Function {
                ...
                compound
                    .field("binder", binder)
                    .field("type", &type_.display(context).as_debug());
                match value {
                    Some(value) => compound.field("value", &value.display(context).as_debug()),
...
impl<C: hir::DisplayContext> hir::Display<C> for BindingRegistration {
    type Output<'a> = impl fmt::Display where C: 'a;

    fn display(&self, context: C) -> Self::Output<'_> {
        formatted(move |f| self.format(context, f))
    }
}
