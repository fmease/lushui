...
use lushui_diagnostics::{reporter::ErasedReportedError, Diagnostic, ErrorCode};
use lushui_entity::EntityKind;
use lushui_error::{Health, Result, Stain};
use lushui_hir::{self as hir, intrinsic, known, Declaration, Display, Expression, Identifier};
use lushui_lowered_ast::AttributeName;
use lushui_session::{Component, IdentifierExt, BuildSession};
use lushui_utilities::{pluralize, QuoteExt};
...
expected type ‘{}’
 but got type ‘{}’",
                    expected.display(self.session.with(self.component)),
                    actual.display(self.session.with(self.component))
                ))
                ...
                return Err(Diagnostic::bug()
                    ...
                    .note(format!(
                        ...
                        self.out_of_order_bindings
                            .iter()
                            .map(|binding| binding
                                .display(self.session.with(self.component))
                                .quote())
                            .join_with(", ")
                    ))
                    .report(self.session.reporter()));
...
expected type ‘_ -> _’
 but got type ‘{}’",
                            type_of_callee.display(self.session.with(self.component))
                        ))
                        .report(self.session.reporter())
                        .into());
                }
            ...
            CaseAnalysis(analysis) => {
                ...
                match &subject_type.bare {
                    ...
                    _ => todo!(
                        ...
                        subject_type.display(self.session.with(self.component))
                    ),
                };
                            ...
                            self.it_is_actual(subject_type.clone(), number_type, scope)
                                .map_err(|error| {
                                    handle_type_mismatch(
                                        error,
                                        self.session.with(self.component),
            ...
            .equals(&type_, &callee, &FunctionScope::Module)?
        {
            ...
            Err(Diagnostic::error()
                ...
                .message(format!(
                    ...
                    result_type.display(self.session.with(self.component)),
                    type_.display(self.session.with(self.component))
                ))
