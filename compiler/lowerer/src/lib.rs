...
#![feature(decl_macro, default_free_fn, slice_take, label_break_value, let_else)]

// @Task ungate named arguments but validate them in the resolver (and/or typer)

use lushui_ast::{self as ast, BareHanger, Explicit, Parameter, Path};
use lushui_diagnostics::{reporter::ErasedReportedError, Diagnostic, ErrorCode, Reporter};
use lushui_error::{Health, OkIfUntaintedExt, PossiblyErroneous, Result, Stain};
use lushui_lowered_ast::{
    self as lowered_ast,
...
use lushui_utilities::{Atom, Conjunction, IOError, ListingExt, QuoteExt, SmallVec, Str};
use smallvec::smallvec;
use std::{default::default, fmt, iter::once};

// @Beacon @Beacon @Beacon @Temporary
// @Task create a constant shared across several crates
    ...
    fn lower_module_declaration(
        ...
        let declarations = match module.declarations {
            ...
            None => {
                ...
                let Ok(declaration) = lushui_syntax::parse_module_file(file, module.binder.clone(), self.session) else {
                    self.health.taint();
                    return PossiblyErroneous::error();
                };
...
pub(crate) trait BareAttributeExt: Sized {
    fn parse(attribute: &ast::Attribute, options: &Options, session: &BuildSession)
        -> Result<Self>;
}
