...
use lushui_ast as ast;
use lushui_error::Result;
use lushui_hir::{self as hir, LocalDeclarationIndex};
use lushui_lowered_ast::{
    attributes::Query as _, Attribute, AttributeName, Attributes, BareAttribute,
};
use lushui_resolver::ComponentExt;
use lushui_session::{Component, ComponentOutline, DeclarationIndexExt, IdentifierExt, BuildSession};
use lushui_utilities::condition;
    ...
    fn persist(&self) -> Result<PathBuf, std::io::Error> {
...

        {
            ...
            if DEVELOPING || !path.exists() {
                static STYLE_SHEET: &str = include_str!("static/css/style.css");
...

        {
            ...
            if !path.exists() {
                static STYLE_SHEET: &str = include_str!("static/css/fonts.css");
                minifier::css::minify(STYLE_SHEET)
                    .unwrap()
                    .write(BufWriter::new(fs::File::create(path)?))?;
            }
...

        {
            ...
            // @Task instead of `DEVELOPING`, compare hash (maybe)?
            if DEVELOPING || !path.exists() {
                static SCRIPT: &str = include_str!("static/js/script.js");
