...
#![feature(default_free_fn, stmt_expr_attributes)]

use lushui_diagnostics::{reporter::ErasedReportedError, Diagnostic, ErrorCode, Reporter};
use lushui_error::{Health, Outcome, Result};
use lushui_lexer_utilities::Lexer as _;
use lushui_session::BuildSession; // @Task get rid of this dep if possible!s
use lushui_span::{LocalSpan, SourceFile, SourceFileIndex, SourceMap, Spanned};
use lushui_token::{BareToken, Provenance, Token, TokenExt, UnterminatedTextLiteral, Word};
use lushui_utilities::{self, GetFromEndExt, obtain};
use std::{
    cmp::Ordering::{self, *},
    default::default,
    fmt,
...
impl<'a> lushui_lexer_utilities::Lexer<'a, BareToken> for Lexer<'a> {
