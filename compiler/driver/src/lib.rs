...
use lushui_diagnostics::{reporter::ErasedReportedError, Diagnostic, ErrorCode, Reporter};
use lushui_error::Result;
use lushui_hir::Display;
use lushui_lexer::WordExt;
use lushui_package::{find_package, resolve_file, resolve_package, MANIFEST_FILE_NAME};
use lushui_resolver::ProgramEntryExt;
use lushui_session::{BuildSession, Component, ComponentOutline, ComponentType, Components};
use lushui_span::SourceMap;
use lushui_token::Word;
use lushui_utilities::IOError;
use std::{
    borrow::Cow,
    default::default,
    io,
...
// @Beacon @Beacon @Beacon @Temporary
// @Task put this in some shared crate
    ...
    match command {
        ...
        Serve => {
            tokio::runtime::Builder::new_multi_thread()
                ...
                .build()
                .unwrap()
                .block_on(lushui_server::serve(map.clone()));
            Ok(())
        }
...
fn build_component(
    ...
    time! {
        ...
        let tokens = lushui_lexer::lex(file, session)?.bare;
    };
    ...
    time! {
        ...
        let component_root = lushui_parser::parse_root_module_file(&tokens, file, session)?;
    }
    ...
    let lowering_options = lushui_lowerer::Options {
    ...
    time! {
        ...
        let component_root =
            lushui_lowerer::lower_file(component_root, lowering_options, component, session)?;
    }
    ...
    time! {
        ...
        let component_root =
            lushui_resolver::resolve_declarations(component_root, component, session)?;
    }

    if options.emit_hir {
        eprintln!("{}", component_root.display(session.with(component)));
    }
    // if component.is_goal(session) && options.emit_untyped_bindings {
    //     eprintln!("{}", component.display(session));
    // }
    // @Beacon @Beacon @Beacon @Task
    ...
    time! {
        ...
        lushui_typer::check(&component_root, component, session)?;
    }
    ...
    match &mode {
        BuildMode::Run { options } => {
            if component.is_goal(session) {
                ...
                match options.backend {
                    Backend::Hiri => {
                        let result =
                            lushui_typer::interpreter::evaluate_main_function(component, session)?;
                        println!("{}", result.display(session.with(component)));
                    }
        ...
        BuildMode::Build { options } => match options.backend {
            ...
            #[cfg(feature = "cranelift")]
            Backend::Cranelift => lushui_cranelift_codegen::compile_and_link(
                lushui_cranelift_codegen::Options {
            ...
            #[cfg(feature = "llvm")]
            Backend::Llvm => {
                lushui_llvm_codegen::compile_and_link(
                    lushui_llvm_codegen::Options {
        ...
        BuildMode::Document { options } => {
            ...
            time! {
                ...
                let index_page_path = lushui_documenter::document(
                    &component_root,
                    options.general,
                    component_outlines,
                    component,
...
fn check_metadata_file(path: &Path, map: &Arc<RwLock<SourceMap>>, reporter: &Reporter) -> Result {
    ...
    lushui_metadata::parse(file, map, reporter).map(drop)
}
