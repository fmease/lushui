...
use lushui_ast::{self as ast, BareHanger};
use lushui_diagnostics::{reporter::ErasedReportedError, Diagnostic, ErrorCode, LintCode};
use lushui_entity::{Entity, EntityKind};
use lushui_error::{Health, OkIfUntaintedExt, PossiblyErroneous, ReportedExt, Result, Stain};
use lushui_hir::{
    self as hir, intrinsic, DeBruijnIndex, DeclarationIndex, Display, Exposure, ExposureReach,
    Identifier, Index, LocalDeclarationIndex, PartiallyResolvedPath,
};
use lushui_lexer::is_punctuation;
use lushui_lowered_ast::{self as lowered_ast, AttributeName, Attributes};
use lushui_session::{Component, DeclarationIndexExt, IdentifierExt, LocalDeclarationIndexExt, BuildSession};
use lushui_span::{Span, Spanned, Spanning};
use lushui_token::Word;
use lushui_utilities::{
    cycle::find_cycles, obtain, pluralize, AsAutoColoredChangeset, Conjunction, HashMap,
    ListingExt, QuoteExt, SmallVec,
};
use smallvec::smallvec;
use std::{cmp::Ordering, collections::VecDeque, default::default, fmt, mem, sync::Mutex};
...
expected the exposure of ‘{}’
      ...
      but it actually is {}",
                            self.component
                                .path_to_string(index.global(self.component), self.session),
                            target.exposure.display(self.session.with(self.component)),
                            entity.exposure.display(self.session.with(self.component)),
                        ))
                        .report(self.session.reporter());
                    self.health.taint();
                }
...
pub trait ComponentExt {
    ...
    fn local_root(&self) -> LocalDeclarationIndex {
        LocalDeclarationIndex::new(0)
    }

    /// The root module / the component root.
    fn root(&self) -> DeclarationIndex;
    ...
    fn path_to_string(&self, index: DeclarationIndex, session: &BuildSession) -> String {
        self.path_with_root_to_string(index, BareHanger::Topmost.name().to_owned(), session)
    }

    // @Beacon @Task make this private again
    ...
    /// [1]: crate::syntax::ast::Path
    // @Task update docs
    fn extern_path_with_root_to_string(&self, index: LocalDeclarationIndex, root: String)
        -> String;

    // @Task add documentation
    fn local_path_segments(&self, index: LocalDeclarationIndex) -> VecDeque<&str>;
...
impl ComponentExt for Component {
    ...
    fn local_path_segments(&self, mut index: LocalDeclarationIndex) -> VecDeque<&str> {
