...
fn parse_components(
    ...
    for (name, untyped_component) in untyped_components.bare {
        ...
        let name =
            parse_name(name.strengthen(), NameKind::Component, reporter).map(Spanned::weaken);
...
fn parse_dependencies(
    ...
    for (component_exonym, declaration) in untyped_dependencies.bare {
        let component_exonym = component_exonym.strengthen().with_text_content_span(map);
        let exonym =
            parse_name(component_exonym, NameKind::Component, reporter).map(Spanned::weaken);
