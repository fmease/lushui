...
use lushui_metadata::Record;
use lushui_session::{BuildSession, Component, ComponentType, Components, Package, PackageIndex, PossiblyUnresolvedComponent, ComponentKey};
use lushui_span::{SourceMap, Spanned, WeaklySpanned};
...
// @Task have a constant shared across several crates
        ...
        let mut package_path = match dependency {
            ...
            Dependency::LocalComponent => {
                    ...
                    .get(&(component_endonym.bare.clone(), ComponentType::Library))
                {
                    ...
                    None => Err(undefined_library_component_error(
                        component_endonym,
                        &package.name,
                    )
                    .report(&self.reporter)
...
fn parse_component_name_from_file_path(path: &Path, reporter: &Reporter) -> Result<Word> {
    if !lushui_utilities::has_file_extension(path, FILE_EXTENSION) {
        Diagnostic::warning()
