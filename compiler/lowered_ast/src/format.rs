...
pub trait Display {
    // @Question do we actually need the lt param here?
    type Output<'a>: fmt::Display + 'a
    where
        Self: 'a;

    fn display(&self) -> Self::Output<'_>;
}

// @Task reduce amount of (String) allocations
impl Display for super::Declaration {
    type Output<'a> = impl fmt::Display;

    fn display(&self) -> Self::Output<'_> {
        formatted(|f| format_declaration(self, 0, f))
    }
}
    ...
    match &declaration.bare {
        Function(function) => {
            write!(
                ...
                "{}{colon} {}",
                function.binder,
                function.type_annotation.display(),
        ...
        Data(type_) => match &type_.constructors {
            Some(constructors) => {
                writeln!(
                    ...
                    binder = type_.binder,
                    colon = ":".color(PUNCTUATION_COLOR),
                    type_ = type_.type_annotation.display(),
                    data = "data".color(KEYWORD_COLOR),
                    of = "of".color(KEYWORD_COLOR)
                )?;
                for constructor in constructors {
                    format_declaration(constructor, depth + 1, f)?;
                }
                Ok(())
            }
            None => writeln!(
        ...
        Constructor(constructor) => {
            writeln!(
                ...
                colon = ":".color(PUNCTUATION_COLOR),
                type_ = constructor.type_annotation.display()
            )
        }
        Module(module) => {
            writeln!(
            ...
            for declaration in &module.declarations {
                format_declaration(declaration, depth + 1, f)?;
            }
            Ok(())
        }
        Use(use_) => writeln!(
...
impl Display for super::Expression {
    ...
    fn display(&self) -> Self::Output<'_> {
        formatted(|f| format_pi_type_literal_or_lower(self, f))
    }
}
    ...
    match &expression.bare {
        ...
        CaseAnalysis(analysis) => {
            write!(
                ...
                subject = analysis.scrutinee.display(),
                of = "of".color(KEYWORD_COLOR)
            )?;
            let mut first = true;
            for case in &analysis.cases {
                ...
                write!(
                    f,
                    "{pattern} {arrow} {body}",
                    pattern = case.pattern.display(),
                    arrow = "=>".color(PUNCTUATION_COLOR),
                    body = case.body.display()
                )?;
            }
            write!(f, " }}")
        }
...
impl Display for super::Pattern {
    ...
    fn display(&self) -> Self::Output<'_> {
        formatted(|f| format_pattern(self, f))
    }
}

// @Task update bracket business
fn format_pattern(pattern: &super::Pattern, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    match &pattern.bare {
        ...
        Application(application) => {
            write!(
                f,
                "({}) ({})",
                application.callee.display(),
                application.argument.display()
            )
        }
        // @Task abstract over fmting sequence literals (via a function over Item<_>)
        // once we have format_lower_pattern
