...
// @Task add tests!
pub(crate) fn find_cycles_by_key<T, U>(
    graph: &HashMap<T, U>,
    get_key: impl Fn(&U) -> &T,
) -> Vec<Cycle<'_, U>>
where
...
pub(crate) type Graph<T> = HashMap<T, T>;
pub(crate) type Cycle<'a, T> = HashSet<&'a T>;

pub(crate) fn find_cycles<T>(graph: &Graph<T>) -> Vec<Cycle<'_, T>>
where
    T: Eq + Hash,
{
    let mut cycles = Vec::new();
