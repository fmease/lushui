...
impl<'scope> TextProcessor<'scope> {
    pub(super) fn new<'a>(
        ...
        Ok(if options.asciidoc {
            Self::AsciiDoctor(Box::new(Asciidoctor::new(folder, crate_, session, scope)?))
        } else {
            Self::None
        })
    }
...
impl<'scope> Asciidoctor<'scope> {
    ...
    pub(super) fn new<'a>(
        ...
        fs::write(
            &extensions_path,
            include_str!("static/rb/AsciidoctorExtensions.rb"),
        )?;
    ...
    fn watch_requests(
        ...
        // @Beacon @Beacon @Beacon @Task use notify::raw_waker instead!
        ...
        Ok(match self {
            Request::DeclarationUrl(path) => {
                ...
                let index = resolve_path(
                    &path,
                    crate_.root().global(crate_),
                    crate_,
                    session,
                    // &SilentReporter.into(),
                    &StderrReporter::new(Some(map)).into(),
                )?;
                let url_suffix = declaration_url_fragment(index, crate_, session);
